...
dcd = models.DCD(h_features=32,input_features=85)
dcd.to(device)

classifier = models.Classifier(85)
...
optimizer_style_discrim = torch.optim.SGD( style_discrim.parameters(), lr = DISCRIM_LR )
optimizer_content_discrim = torch.optim.SGD( content_discrim.parameters(), lr = DISCRIM_LR )
...
for iter in range(1, NUM_ITERS+1):
    ...
    '''
    content_labels = torch.cat((mnist_label, svhn_label), 0)
    ...
    dcd_labels = torch.cat((rs_rc_labels,rs_sc_labels,ss_rc_labels,ss_sc_labels), 0)

    '''
    Step 4) Train DCD Adversarially
    '''
    joined_encoded = torch.cat((rs_rc_encoded, rs_sc_encoded, ss_rc_encoded, ss_sc_encoded), 0)
    joint_discrim_preds = dcd(joined_encoded.detach())
    dcd_discrim_loss = cls_criterion(joint_discrim_preds, dcd_labels)

    dcd_adv_labels = torch.cat((ss_rc_labels, ss_sc_labels, rs_rc_labels,  rs_sc_labels  ), 0)
    joint_discrim_preds = dcd(joined_encoded )
    dcd_adv_loss = cls_criterion(joint_discrim_preds, dcd_adv_labels)
    ...
    rs_sc_loss = cls_criterion(rs_sc_preds, svhn_label)
    ss_rc_loss = cls_criterion(ss_rc_preds, mnist_label)
    ss_sc_loss = cls_criterion(ss_sc_preds, svhn_label)

    cls_loss = (rs_rc_loss + rs_sc_loss + ss_rc_loss + ss_sc_loss) / 4.
    ...
    running_dcd_loss += dcd_discrim_loss.item()
    running_dcd_adv_loss += dcd_adv_loss.item()

    style_discriminator_loss *= .05
    content_discriminator_loss *= .05
    dcd_discrim_loss *= 0
    style_adversarial_loss *= .05
    content_adversarial_loss *=  .05
    dcd_adv_loss *= 0
    cls_loss *= 1.0
