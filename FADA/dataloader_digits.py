...
# extract labeled target data according to the given number
def create_target_samples(is_M2S, n):
    if is_M2S:
        dataset = datasets.SVHN('./data/SVHN', split='train', download=True,
                                transform=transforms.Compose([
                                    transforms.Resize((28, 28)),
...
G3: same domain, different classes

G2: different domain,same class
G4: different domain, different classes
"""
def create_groups(X_s, Y_s, X_t, Y_t, seed=1, num_samples=7):
    ...
    for i in range(10):
        for j in range(shot):
            ...
            if num_samples > 1:
                ...
                G4.append((X_s[source_matrix[i % 10][j]], X_t[target_matrix[(i + 1) % 10]]))
                Y4.append((Y_s[source_matrix[i % 10][j]], Y_t[target_matrix[(i + 1) % 10]]))
...
def sample_groups(X_s, Y_s, X_t, Y_t, seed=1, num_samples=7):
